 ...
 * 启用、禁用规则
 */
public class RequestRuleSwitch implements HttpUtils.OnHttpRequestCallBack {

    private Context context;
    private CommentRequestCallback mCommentRequestCallback;

    public static RequestRuleSwitch getInstance() {
        return new RequestRuleSwitch();
    }
     ...
     */
    public void switchRule(Context context, int ruleId, int status, CommentRequestCallback mCommentRequestCallback) {
        ...
        try {
            ...
            if (UserUtils.isLogin(context))
                HttpUtils.getInstance().putRequestInfo(NetConfig.URL_RULE_SWITCH + ruleId + "/changestatus?access_token=" + UserUtils.getAccessToken(context),
                        params, null, this);
