...
public class SetupStepsFactory {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public SaveDeviceStep newSaveDeviceStep(ParticleCloud sparkCloud, String deviceId, Context context) {
        return new SaveDeviceStep(
                StepConfig.newBuilder()
                        .setMaxAttempts(MAX_RETRIES_CLAIM)
                        .setResultCode(SuccessActivity.RESULT_FAILURE_CLAIMING)
                        .setStepId(R.id.save_device)
                        .build(),
                sparkCloud, deviceId, context);
    }
}
